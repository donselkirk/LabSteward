...
bash -n labsteward.sh ct/labsteward.sh install/labsteward-install.sh src/self-update.sh \
  tests/mock-systemctl.sh tools/build-artifacts.sh tools/build-release-assets.sh tools/fetch-community-helpers.sh
...
grep -q 'optional_runtime_assets=(labsteward-core.py labsteward-mcp.py labsteward.service)' src/self-update.sh
...
if grep -q 'X-Forwarded-For' src/labsteward_mcp.py; then
